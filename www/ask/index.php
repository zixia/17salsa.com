...
$count_show_share=intval($count_show_share);
...
$query=$dblink->query("select qid,title,sid1,sid2,sid3 from {$tablepre}question where status=1 order by qid desc limit $count_show_nosolve");
$nosolve_ques=array();
...
$query=$dblink->query("select qid,title,sid1,sid2,sid3 from {$tablepre}question where status=7 order by qid desc limit $count_show_share");
$knowledge_sharing=array();
while($solve=$dblink->fetch_array($query))
{
	$solve['url'] = $htmlopen == 1 ? 'question/'.$solve['qid'].'.html' : 'question.php?qid='.$solve['qid'];
	...
	$sort=$dblink->fetch_array($query_sort);
	$knowledge_sharing[] = array_merge($solve,$sort);
}
