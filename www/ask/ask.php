...
if($command=='ask')
{
	...
	if(check_submit($_POST['submit'], $_POST['formhash']))
	{
		...
		if($give_score)
		{
			...
			{
				update_score($cyask_uid,$give_score,'-'); //扣分
			}
		}
		...
		$ques_title = $dblink->escape_string($ques_title);
		$ques_supplement = $dblink->escape_string($ques_supplement);
		...
		if($do)
		{
			...
			$title_template = '{actor} 提出问题：“{qtitle} ”';
			
			$feed_content = '<a href="'.$boardurl.'question.php?qid='.$qid.'" target="_blank">'.$ques_title.'</a>';
