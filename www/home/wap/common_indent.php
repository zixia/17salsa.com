...
@define ('qVh0gqGnK', TRUE);
@define ('agF2gTdKE', "wap");
define ('D_BUG', '0');
D_BUG ? error_reporting (7) : error_reporting (0);
set_magic_quotes_runtime (0);
error_reporting (7);
$_SGLOBAL = $_SCONFIG = $_SBLOCK = $_TPL = $_SCOOKIE = $_SN = $space = $KGn1g4iOB = array ();
define ('XnP3g6CaJ', dirname (__FILE__).DIRECTORY_SEPARATOR);
define ('S_ROOT', str_replace(agF2gTdKE.'/common.php', '', str_replace ('\\', '/', __FILE__)));
...
if (empty ($_SC['charset_wap']))
  {
    $_SC['charset_wap'] = $_SC['charset'];
  }
include_once (XnP3g6CaJ.'./source/function_common.php');
...
if (!@include_once (S_ROOT.'./data/data_config.php'))
  {
    include_once (WAP_BASE.'./source/function_cache.php');
    config_cache ();
...
foreach ($_COOKIE as $key = >$val)
{
  if (substr ($key, 0, $prelength) == $_SC['cookiepre'])
    {
      $_SCOOKIE[(substr ($key, $prelength))] =
	empty ($magic_quote) ? saddslashes ($val) : $val;
    }
}
...
$_SGLOBAL['ajaxmenuid'] =
empty ($_GET['ajaxmenuid']) ? '' : $_GET['ajaxmenuid'];
$_SGLOBAL['refer'] =
empty ($_SERVER['HTTP_REFERER']) ? '' : $_SERVER['HTTP_REFERER'];
$_SGLOBAL['mobile'] = '1';
if (!empty ($_SGLOBAL['refer']))
  {
    $poGfgcg4G = parse_url ($_SGLOBAL['refer']);
    if ($poGfgcg4G)
      {
	$_SGLOBAL['back_refer'] =
	  basename ($poGfgcg4G['path'])."?".$poGfgcg4G['query'];
      }
  }
...
if ($_SCOOKIE['mytemplate'])
  {
    $_SCOOKIE['mytemplate'] =
      str_replace ('.', '', trim ($_SCOOKIE['mytemplate']));
    if (file_exists (XnP3g6CaJ.'./tpl/'.$_SCOOKIE['mytemplate'].'/style.css'))
      {
...
checkauth ();
$_SGLOBAL['uhash'] =
md5 ($_SGLOBAL['supe_uid']."\t".substr ($_SGLOBAL['timestamp'], 0, 6));
getuserapp ();
