...
function authcode ($string, $operation = 'DECODE', $key = '', $expiry = 0)
{
  ...
  $keyc =
    $ckey_length ? ($operation ==
		    'DECODE' ? substr ($string, 0,
				       $ckey_length) :
  ...
  $string =
    $operation ==
    'DECODE' ? base64_decode (substr ($string, $ckey_length)) :
    sprintf ('%010d',
...
} function ssetcookie ($var, $value, $life = 0)
{
  ...
  $_SESSION['m_'.$var] = $value;
  setcookie ($_SC['cookiepre'].$var, $value,
	     $life ? ($_SGLOBAL['timestamp'] + $life) : 0, $_SC['cookiepath'],
	     $_SC['cookiedomain'], $_SERVER['SERVER_PORT'] == 443 ? 1 : 0);
} function dbconnect ()
{
  global $_SGLOBAL, $_SC;
  include_once (XnP3g6CaJ.'./source/class_mysql.php');
  if (empty ($_SGLOBAL['db']))
    {
      $_SGLOBAL['db'] = new dbstuff;
      $_SGLOBAL['db']->rZJ6glaFc = $_SC['dbcharset'];
      $_SGLOBAL['db']->charset_wap = $_SC['charset_wap'];
      $_SGLOBAL['db']->connect ($_SC['dbhost'], $_SC['dbuser'], $_SC['dbpw'],
				$_SC['dbname'], $_SC['pconnect']);
    }
}
...
function checkauth ()
{
  ...
  $_SGLOBAL['supe_auth'] = "t\x30";
  if ($_GET['m_sid'])
    $_SCOOKIE['auth'] = $_SESSION['m_auth'];
  if ($_SCOOKIE['auth'])
    {
      @list ($password, $uid) =
	explode ("\t", authcode ($_SCOOKIE['auth'], 'DECODE'));
      $_SGLOBAL['supe_uid'] = intval ($uid);
      if ($password && $_SGLOBAL['supe_uid'])
	{
	  $query =
	    $_SGLOBAL['db']->
	    query ("\x53\105\x4c\105\x43\x54 * \106\122O\x4d ".
		   tname ('session')." WHERE uid='$_SGLOBAL[supe_uid]'");
	  if ($member = $_SGLOBAL['db']->fetch_array ($query))
	    ...
	    {
	      $query =
		$_SGLOBAL['db']->query ("S\105\114E\x43T * FR\117\x4d ".
					tname ('member').
  ...
  $JRHdg7niZ =
    "/[\w-]+\.(c\x6f\x6d|net|org|gov|cc|\142\151\172|info|cn)(\.(cn|h\x6b))*/";
  if (@preg_match ($JRHdg7niZ, $KGn1g4iOB['SERVER_NAME'], $match))
    {
      $KGn1g4iOB['SERVER_NAME'] = $match[0];
    }
  if ((md5 ($KGn1g4iOB['SERVER_NAME'].$KGn1g4iOB['SERVER_PORT']).
       md5 ($KGn1g4iOB['SERVER_SOFTWARE'].$_SC['WAP_TYPE'])) ==
      $_SC['WAP_LICENCE'])
    {
  ...
  if ($_SGLOBAL['supe_uid'] && $_SCONFIG['my_status'])
    {
      ...
      $showcount = 0;
      $query =
	$_SGLOBAL['db']->query ("\123E\114\105CT * \106\122O\x4d ".
				tname ('userapp').
				" WHERE uid='$_SGLOBAL[supe_uid]' ORDER BY menuorder DESC",
				'SILENT');
      while ($value = $_SGLOBAL['db']->fetch_array ($query))
	{
	  $_SGLOBAL['my_userapp'][$value['appid']] = $value;
	  if ($value['allowsidenav']
	      && !isset ($_SGLOBAL['userapp'][$value['appid']]))
	    {
	      if ($space['menunum'] < 5)
		$space['menunum'] = 10;
		       ...
		       array ())
{
  ...
  elseif ($msgkey ==
	  "\141uth_\x75\x6eall\157\167e\x64_\146u\x6ec\x74\x69\x6f\x6e")
  {
    $message =
      "t\x68is \151s not \141\x75t\x68o\162iz\x61\x74\151\157n \142\171 author,\160\154\x65as\145 send email to 34625376@qq.co\155";
  }
  else
  {
    $message = $msgkey;
  ...
  if (empty ($_SGLOBAL['supe_auth']) || $_SGLOBAL['supe_auth'] == 't0')
    {
      $message =
	"\x74\150\x69s i\x73 n\157\x74 \x61\x75\164\150\157\x72\151\x7a\x61\x74\x69\157\156 \x62\x79 author,\160l\x65\x61\163\145 send email to 34625376@qq.\x63o\155";
    }
...
function inserttable ($tablename, $insertsqlarr, $returnid = 0, $replace =
		       false, $silent = 0)
{
  global $_SGLOBAL;
  ...
  $_SGLOBAL['db']->query ($method.' INTO '.tname ($tablename).' ('.
...
function updatetable ($tablename, $setsqlarr, $wheresqlarr, $silent = 0)
{
  ...
  $_SGLOBAL['db']->query ('UPDATE '.tname ($tablename).' SET '.$setsql.
			  ' WHERE '.$where, $silent ? 'SILENT' : '');
}
  ...
  if (empty ($_SGLOBAL[$var]))
    {
      $space = array ();
      $query =
	$_SGLOBAL['db']->query ("\123\x45\x4cEC\x54 sf.*, s.* \x46\x52\x4fM ".
				tname ('space')." s LEFT JOIN ".
				tname ('spacefield').
				" sf \117\x4e sf.uid=s.uid WHERE s.{$indextype}='$key'");
      if (!$space = $_SGLOBAL['db']->fetch_array ($query))
	{
	  ...
	  if ($indextype == 'uid' && $auto_open)
	      ...
	      if ($user = uc_get_user ($key, 1))
		{
		  include_once (S_ROOT.'./source/function_space.php');
		  $space =
		    space_open ($user[0], addslashes ($user[1]), 0,
				addslashes ($user[2]));
		}
	    }
	}
      if ($space)
	{
	  $_SN[$space['uid']] = ($_SCONFIG['realname'] && $space['name']
				 && $space['namestatus']) ? $space['name'] :
	    $space['username'];
	  ...
	  if (empty ($space['friend']))
	      ...
	      if ($space['friendnum'] > 0)
		{
		  ...
		  $query =
		    $_SGLOBAL['db']->
		    query ("\123\x45\114E\x43T fuid \x46\122\117M ".
			   tname ('friend').
			   " WHERE uid='$space[uid]' AND \x73\164\x61\164\x75\163='1'");
		  while ($value = $_SGLOBAL['db']->fetch_array ($query))
		    {
	  ...
	  $space['privacy'] =
	    empty ($space['privacy']) ? (empty ($_SCONFIG['privacy']) ?
					 array () : $_SCONFIG['privacy']) :
	    unserialize ($space['privacy']);
	  $space['allnotenum'] = 0;
	  foreach (array
		   ('notenum', 'pokenum', 'addfriendnum', 'mtaginvitenum',
		    'eventinvitenum', 'myinvitenum') as $value)
	  {
...
function getuid ($name)
{
  ...
  $query =
    $_SGLOBAL['db']->
    query ("\x53E\114E\103T uid,username,name,namestatus \106R\x4f\x4d ".
	   tname ('space')." WHERE ".implode (' OR ', $wherearr)." LIMIT 1");
  if ($space = $_SGLOBAL['db']->fetch_array ($query))
    {
      $uid = $space['uid'];
    }
  ...
  else
  {
    ...
    if (!$result && $space['privacy']['view'][$type] == 1)
	...
	if (!isset ($space['isfriend']))
	  {
	    ...
	    if ($space['friends']
		&& in_array ($_SGLOBAL['supe_uid'], $space['friends']))
	      {
		$space['isfriend'] = 1;
...
function getgroupid ($experience, $gid = 0)
{
  ...
  if ($gid)
      ...
      if (@include_once (S_ROOT.'./data/data_usergroup_'.$gid.'.php'))
	{
	  ...
	  if (empty ($group['system']))
	    {
	      if ($group['exphigher'] < $experience
		  || $group['explower'] > $experience)
		{
		  $needfind = true;
		}
	    }
  ...
  if ($needfind)
    {
      $query =
	$_SGLOBAL['db']->query ("S\105\x4c\105\103T gid \x46\x52O\x4d ".
				tname ('usergroup').
				" WHERE explower<='$experience' AND \163yste\155='0' ORDER BY explower DESC LIMIT 1");
  ...
  if (!isset ($_SGLOBAL[$var]))
      ...
      else
	{
	  if (empty ($_SGLOBAL['member']))
	    getmember ();
	  $gid =
	    getgroupid ($_SGLOBAL['member']['experience'],
			$_SGLOBAL['member']['groupid']);
	  @include_once (S_ROOT.'./data/data_usergroup_'.$gid.'.php');
	  if ($gid != $_SGLOBAL['member']['groupid'])
	    {
	      updatetable ('space', array ('groupid' = >$gid),
...
function runlog ($file, $log, $halt = 0)
{
  ...
  $log =
    sgmdate ('Y-m-d H:i:s',
	     $_SGLOBAL['timestamp'])."\t$type\t".getonlineip ().
    "\t$_SGLOBAL[supe_uid]\t{$nowurl}\t".str_replace (array ("\r", "\n"),
						      array (' ', ' '),
						      trim ($log))."\n";
...
function getstr ($string, $length, $in_slashes = 0, $out_slashes =
		  0, $censor = 0, $bbcode = 0, $html = 0)
{
  global $_SC, $_SGLOBAL;
  $string = trim ($string);
  if ($in_slashes)
  ...
  if ($censor)
    {
	  ...
	  && preg_match ($_SGLOBAL['censor']['banned'], $string))
	{
	...
	{
	  $string =
	    empty ($_SGLOBAL['censor']['filter']) ? $string :
	    @preg_replace ($_SGLOBAL['censor']['filter']['find'],
			   $_SGLOBAL['censor']['filter']['replace'], $string);
	}
    }
...
function sgmdate ($dateformat, $timestamp = '', $format = 0)
{
  ...
  $timeoffset =
    strlen ($_SGLOBAL['member']['timeoffset']) >
    0 ? intval ($_SGLOBAL['member']['timeoffset']) :
    intval ($_SCONFIG['timeoffset']);
  $result = '';
  if ($format)
    {
  ...
  if ($num > $perpage)
    {
      ...
      $pages = $_SCONFIG['maxpage']
	&& $_SCONFIG['maxpage'] <
	$realpages ? $_SCONFIG['maxpage'] : $realpages;
      if ($page > $pages)
	{
	  $from = 1;
	  $to = $pages;
      ...
      if ($curpage - $offset > 1 && $pages > $page)
	{
	  ...
	  if ($_SGLOBAL['inajax'])
	    {
	      $multipage. =
		"href=\"javascript:;\" onclick=\"ajaxget('{$mpurl}page=1&ajaxdiv=$ajaxdiv', '$ajaxdiv')\"";
	    }
	  else
	    {
	      $multipage. = "href=\"{$mpurl}page=1{$urlplus}\"";
	    }
	  $multipage. = " \x63l\x61\x73s=\"first\">1 ...</a>";
	}
      if ($curpage > 1)
	{
	  $multipage. = "<a ";
	  if ($_SGLOBAL['inajax'])
	    {
	      $multipage. =
		"href=\"javascript:;\" onclick=\"ajaxget('{$mpurl}page=".
		($curpage - 1)."&ajaxdiv=$ajaxdiv', '$ajaxdiv')\"";
	    }
	  else
	    {
	      $multipage. =
		"href=\"{$mpurl}page=".($curpage - 1)."$urlplus\"";
	    }
	  $multipage. = " c\x6c\x61\x73\x73=\"prev\">&lsaquo;&lsaquo;</a>";
	}
      for ($i = $from; $i <= $to; $i++)
	{
	  if ($i == $curpage)
		...
		{
		  $multipage. =
		    "href=\"javascript:;\" onclick=\"ajaxget('{$mpurl}page=$i&ajaxdiv=$ajaxdiv', '$ajaxdiv')\"";
		}
	      else
		{
      ...
      if ($curpage < $pages)
	{
	  ...
	  $multipage. =
	    " \143\x6c\x61\163\x73=\"\156\x65x\x74\">&rsaquo;&rsaquo;</a>";
	}
      if ($to < $pages)
	{
	  $multipage. = "<a ";
	  if ($_SGLOBAL['inajax'])
	    {
	      $multipage. =
		"href=\"javascript:;\" onclick=\"ajaxget('{$mpurl}page=$pages&ajaxdiv=$ajaxdiv', '$ajaxdiv')\"";
	    }
	  else
	    {
	      $multipage. = "href=\"{$mpurl}page=$pages{$urlplus}\"";
	    }
	  $multipage. = " \143\x6c\x61\163\x73=\"last\">... $realpages</a>";
	}
      if ($multipage)
	{
...
{
  global $_SGLOBAL;
  $multi = array ('last' = >-1, 'next' = >-1, 'begin' = >-1, 'end' =
		  >-1, 'html' = >'');
  if ($start > 0)
    {
      if (empty ($count))
  ...
  if ($multi['begin'] >= 0)
      ...
      if ($multi['last'] >= 0)
	{
	  ...
	  if ($_SGLOBAL['inajax'])
	    {
	      $multi['html']. =
		"<a href=\"javascript:;\" onclick=\"ajaxget('$url&ajaxdiv=$ajaxdiv', '$ajaxdiv')\">|&lt;</a> <a href=\"javascript:;\" onclick=\"ajaxget('$url&start=$multi[last]&ajaxdiv=$ajaxdiv', '$ajaxdiv')\">&lt;</a> ";
	    }
	  else
	    {
	      $multi['html']. =
		"<a href=\"$url\">|&lt;</a> <a href=\"$url&start=$multi[last]\">&lt;</a> ";
	    }
	}
      else
	{
      ...
      if ($multi['next'] >= 0)
	{
	  ...
	  if ($_SGLOBAL['inajax'])
	    {
	      $multi['html']. =
		" <a href=\"javascript:;\" onclick=\"ajaxget('$url&start=$multi[next]&ajaxdiv=$ajaxdiv', '$ajaxdiv')\">&gt;</a> ";
	    }
	  else
	    {
	      $multi['html']. =
		" <a href=\"$url&start=$multi[next]\">&gt;</a>";
	    }
	}
      else
	{
...
function template ($name)
{
  ...
  if (strexists ($name, '/'))
    ...
    {
      $tpl = agF2gTdKE."/tpl/$_SCONFIG[template]/$name";
    }
  $objfile = S_ROOT.'./data/tpl_cache/'.str_replace ('/', '_', $tpl).'.php';
  if (!file_exists ($objfile))
    {
      include_once (XnP3g6CaJ.'./source/function_template.php');
      parse_template ($tpl);
    }
  return $objfile;
}
...
function subtplcheck ($subfiles, $mktime, $tpl)
{
  global $_SC, $_SCONFIG;
  if ($_SC['tplrefresh']
      && ($_SC['tplrefresh'] == 1 || mt_rand (1, $_SC['tplrefresh']) == 1))
    {
	...
	if (!file_exists ($tplfile))
	  {
	    $tplfile =
	      str_replace ('/'.$_SCONFIG['template'].'/', '/default/',
			   $tplfile);
	  }
...
} function getcount ($tablename, $wherearr = array (), $get = 'COUNT(*)')
{
  ...
  return $_SGLOBAL['db']->result ($_SGLOBAL['db']->
				  query ("\x53E\114\105CT $get F\x52\x4fM ".
					 tname ($tablename).
					 " WHERE $wheresql LIMIT 1"), 0);
}
...
function ob_out ()
{
  ...
  $preg_searchs = $preg_replaces = $str_searchs = $str_replaces = array ();
    ...
    "/(href\=\"|action\=\"|;u\162l\=|src\=\")+([a-z0-9]+?)\.\x70h\160\?([a-z0-9\=\&\-\%]+?)\"/ie";
  $preg_replaces[] = 'session_url(\'\\1\',\'\\2\',\'\\3\')';
  $preg_searchs[] =
    "/(href\=\"|action\=\"|;ur\x6c\=|src\=\")+([a-z0-9]+?)\.ph\x70\"/ie";
  $preg_replaces[] = 'session_url(\'\\1\',\'\\2\')';
  if ($_SGLOBAL['supe_auth'] == "\x740" || $_SGLOBAL['supe_auth'] == "t1")
    {
      $str_searchs[] = '</body>';
      $str_replaces[] =
	"</body> ";
  ...
  obclean ();
  $content = jTtcgMMUE ($content, $_SC['charset_wap'], $_SC['dbcharset']);
  if ($_SGLOBAL['inajax'])
    {
      xml_out ($content);
    }
	...
	{
	  @header ('Content-Type: application/vnd.wap.xhtml+xml; charset='.
		   $_SC['charset_wap']);
	}
      echo "<?xml version=\"1.0\" encoding=\"".$_SC['charset_wap']."\"?>";
      echo $content;
      if (D_BUG)
	{
...
xml_out ($content)
{
  ...
  @header ("Expires: -1");
  @header
    ("\103\x61\143\x68\145-Control: n\157-store, \x70\162\x69\166\x61\164\145, post-check=0, pre-check=0, \155\x61x-age=0",
     FALSE);
  @header ("Pragma: \156o-cache");
  @header
    ("\x43\157nt\x65n\x74-type: application/xml; charset=$_SC[charset]");
...
function iframe_url ($url)
{
  ...
  return "<a href=\"li\x6ek.\x70\x68p?u\162\x6c=http://$url\"";
}

function session_url ($type, $name, $para = "")
{
  global $_SGLOBAL, $_SC, $_SERVER;
  if (!empty ($para))
    {
  ...
  if (!defined ('WAP_CONTROL'))
    {
      $name. = ".p\x68p?\155_\163id=".session_id ();
    }
  else
    {
      $name = "wap.\160hp?m_f=$name&\x6d_\163\x69d=".session_id ();
    }
  return stripslashes ($type).$name.$para.'"';
}
...
function cksearch ($theurl)
{
  ...
  if ($searchinterval = checkperm ('searchinterval'))
    {
      $waittime =
	$searchinterval - ($_SGLOBAL['timestamp'] - $space['lastsearch']);
      if ($waittime > 0)
	{
	  showmessage ('search_short_interval', '', 1,
		       array ($waittime, $theurl));
	}
    }
  if (!checkperm ('searchignore'))
      ...
      if ($reward['credit'] || $reward['experience'])
	{
	  if (empty ($_GET['confirm']))
	    {
	      $theurl. = '&confirm=yes';
	      showmessage ('points_deducted_yes_or_no', '', 1,
			   array ($reward['credit'], $reward['experience'],
				  $theurl));
	    }
	  else
	    {
	      ...
	      else
		{
					   ...
					   " SET lastsearch='$_SGLOBAL[timestamp]', credit=credit-$reward[credit], experience=experience-$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
		}
	    }
	}
    }
...
function isholddomain ($domain)
{
  ...
  if (preg_match ("/^[^a-z]/i", $domain))
    return true;
  $holdmainarr =
    empty ($_SCONFIG['holddomain']) ? array ('www') : explode ('|',
							       $_SCONFIG
							       ['holddomain']);
  $ishold = false;
...
function debuginfo ()
{
  ...
  if (empty ($_SCONFIG['debuginfo']))
  ...
  else
    {
      ...
      $totaltime =
	number_format (($mtime[1] + $mtime[0] - $_SGLOBAL['supe_starttime']),
		       4);
      $info =
	'Processed in '.$totaltime.' second(s), '.$_SGLOBAL['db']->querynum.
	' queries'.($_SC['gzipcompress'] ? ', Gzip enabled' : NULL);
    }
  return $info;
}
...
function formatsize ($size)
{
  $prec = 3;
  $size = round (abs ($size));
  $units = array (0 = >" \x42 ", 1 = >" KB", 2 = >" MB", 3 = >" GB", 4 =
		  >" \124\102");
  if ($size == 0)
...
{
  PHP_VERSION <
    '4.2.0' ? mt_srand ((double) microtime () * 1000000) : mt_srand ();
  $seed =
    base_convert (md5 (print_r ($_SERVER, 1).microtime ()), 16,
		  $numeric ? 10 : 35);
  $seed =
    $numeric ? (str_replace ('0', '', $seed).'012340567890') : ($seed.'zZ'.
								strtoupper
								($seed));
  $hash = '';
  $max = strlen ($seed) - 1;
...
function data_get ($var, $isarray = 0)
{
  global $_SGLOBAL;
  $query =
    $_SGLOBAL['db']->query ("SELE\103T * F\x52O\x4d ".tname ('data').
			    " WHERE \x76\x61\162='$var' LIMIT 1");
...
function data_set ($var, $datavalue, $clean = 0)
{
  ...
  if ($clean)
    {
      $_SGLOBAL['db']->query ("DELETE \106\x52\x4fM ".tname ('data').
			      " WHERE \166\x61r='$var'");
    }
  else
    {
	...
	$datavalue = serialize (sstripslashes ($datavalue));
      $_SGLOBAL['db']->query ("REPLACE INTO ".tname ('data').
			      " (\166a\162, datavalue, dateline) VALUES ('$var', '".
			      addslashes ($datavalue).
			      "', '$_SGLOBAL[timestamp]')");
    }
}
...
{
  global $_SGLOBAL, $_SCONFIG;
  if ($_SCONFIG['close'] && !ckfounder ($_SGLOBAL['supe_uid'])
       && !checkperm ('closeignore'))
    {
  ...
  if ((!ipaccess ($_SCONFIG['ipaccess']) || ipbanned ($_SCONFIG['ipbanned']))
       && !ckfounder ($_SGLOBAL['supe_uid']) && !checkperm ('closeignore'))
    {
...
function adshow ($pagetype)
{
  ...
  if (empty ($_SGLOBAL['ad']) || empty ($_SGLOBAL['ad'][$pagetype]))
    return false;
...
function siconv ($str, $out_charset, $in_charset = '')
{
  global $_SC;
  $in_charset =
    empty ($in_charset) ? strtoupper ($_SC['charset_wap']) :
  ...
  if ($in_charset != $out_charset)
    {
	  ...
	  && (@$outstr =
	      iconv ("$in_charset//IG\116\117\x52E",
		     "$out_charset//\111\x47\116O\122E", $str)))
	{
	  return $outstr;
	}
...
function getpassport ($username, $password)
{
  ...
  $username = jTtcgMMUE ($username, $_SC['dbcharset'], $_SC['charset_wap']);
...
function interval_check ($type)
{
  ...
  if ($interval = checkperm ($intervalname))
    {
      $lasttime =
	isset ($space[$lastname]) ? $space[$lastname] : getcount ('space',
								  array ('uid'
									 =
									 >$_SGLOBAL
									 ['supe_uid']),
...
function getstar ($experience)
{
  ...
  if ($_SCONFIG['starcredit'] > 1)
      ...
      if ($_SCONFIG['starlevelnum'] < 2)
	{
	  ...
	  for ($i = 0; $i < $starnum; $i++)
	    {
	      $starimg. =
		'<img src="image/star_level10.gif" align="absmiddle" />';
	    }
	}
      else
	{
	    ...
	    {
	      $numlevel =
		intval ($starnum / pow ($_SCONFIG['starlevelnum'], ($i - 1)));
	      if ($numlevel > 10)
		$numlevel = 10;
	      if ($numlevel)
		{
  ...
  if (empty ($starimg))
    $starimg =
      '<img src="image/credit.gif" alt="'.$experience.
      '" align="absmiddle" alt="'.$experience.'" title="'.$experience.'" />';
  return $starimg;
}
...
function getfriendstatus ($uid, $fuid)
{
  ...
  $query =
    $_SGLOBAL['db']->
    query ("\x53E\114E\x43\124 \x73\x74\x61tu\x73 \106R\117\115 ".
	   tname ('friend')." WHERE uid='$uid' AND fuid='$fuid' LIMIT 1");
  if ($value = $_SGLOBAL['db']->fetch_array ($query))
    {
      return $value['status'];
    }
...
function ckfriend ($touid, $friend, $target_ids = '')
{
  ...
  switch ($friend)
	...
	{
	  $_SGLOBAL[$var] =
	    getfriendstatus ($_SGLOBAL['supe_uid'],
			     $touid) == 1 ? true : false;
	}
      break;
    case 2:
...
function mkfeed ($feed, $actors = array ())
{
  ...
  $feed['title_data'] =
    empty ($feed['title_data']) ? array () :
    unserialize ($feed['title_data']);
  if (!is_array ($feed['title_data']))
    $feed['title_data'] = array ();
  $feed['body_data'] =
    empty ($feed['body_data']) ? array () : unserialize ($feed['body_data']);
  ...
  $searchs[] = '{actor}';
  $replaces[] =
    empty ($actors) ? "<a href=\"space.p\150\x70?uid=$feed[uid]\">".
    $_SN[$feed['uid']]."</a>" : implode (lang ('dot'), $actors);
  $searchs[] = '{app}';
  ...
  $feed['title_template'] =
    mktarget (str_replace ($searchs, $replaces, $feed['title_template']));
  ...
  if ($feed['appid'])
	...
	{
	  $feed['magic_class'] =
	    'magiccolor'.$feed['body_data']['magic_color'];
	}
  ...
  $searchs[] = '{actor}';
  $replaces[] = "<a href=\"space.p\150p?uid=$feed[uid]\">$feed[username]</a>";
  $feed['body_template'] =
    mktarget (str_replace ($searchs, $replaces, $feed['body_template']));
  $feed['body_general'] = mktarget ($feed['body_general']);
    ...
    {
      $feed['icon_image'] =
	"http://appicon.manyou.\x63\157\155/icons/{$feed['icon']}";
    }
  $feed['style'] = $feed['target'] = '';
  if ($_SCONFIG['feedread'] && empty ($feed['id']))
    {
      $read_feed_ids =
	empty ($_COOKIE['read_feed_ids']) ? array () : explode (',',
								$_COOKIE
								['read_feed_ids']);
      if ($read_feed_ids && in_array ($feed['feedid'], $read_feed_ids))
	{
	  $feed['style'] = " c\x6c\141s\163=\"feedread\"";
	}
      else
	{
	  $feed['style'] =
	    " onclick=\"readfeed(t\x68i\163, $feed[feedid]);\"";
	}
    }
  if ($_SCONFIG['feedtargetblank'])
    {
      $feed['target'] = ' target="_blank"';
       ...
       ($feed['idtype'], array ('blogid', 'picid', 'sid', 'pid', 'eventid')))
    {
      $feed['showmanage'] = 1;
    }
...
function mktarget ($html)
{
  ...
  if ($html && $_SCONFIG['feedtargetblank'])
    {
      $html =
	preg_replace ("/<a(.+?)href=([\'\"]?)([^>\s]+)\\2([^>]*)>/i",
		      '<a target="_blank" \\1 href="\\3" \\4>', $html);
    }
  return $html;
}
...
function mkshare ($share)
{
  ...
  $share['body_template'] =
    str_replace ($searchs, $replaces, $share['body_template']);
  return $share;
}

function ipaccess ($ipaccess)
{
  return empty ($ipaccess) ? true : preg_match ("/^(".
						str_replace (array
							     ("\r\n", ' '),
							     array ('|', ''),
							     preg_quote
...
{
  return empty ($ipbanned) ? false : preg_match ("/^(".
						 str_replace (array
							      ("\r\n", ' '),
							      array ('|', ''),
							      preg_quote
...
function avatar ($uid, $size = 'small', $returnsrc = FALSE)
{
  global $_SCONFIG, $_SN;
  $size =
    in_array ($size, array ('big', 'middle', 'small')) ? $size : 'small';
  $avatarfile = avatar_file ($uid, $size);
  return $returnsrc ? UC_API.'/data/avatar/'.$avatarfile : '<img src="'.
    UC_API.'/data/avatar/'.$avatarfile.
    '" onerror="this.onerror=null;this.src=\''.UC_API.'/images/noavatar_'.
    $size.'.gif\'">';
}
...
function getfriendgroup ()
{
  ...
  $groups = array ();
  $spacegroup =
    empty ($space['privacy']['groupname']) ? array () :
...
function sub_url ($url, $length)
{
  if (strlen ($url) > $length)
    {
      $url =
	str_replace (array ('%3A', '%2F'), array (':', '/'),
		     rawurlencode ($url));
      $url =
	substr ($url, 0, intval ($length * 0.5)).' ... '.substr ($url,
								 -intval
								 ($length *
								  0.3));
    }
...
function realname_set ($uid, $username, $name = '', $namestatus = 0)
{
  ...
  {
    $_SN[$uid] = $username;
    $_SGLOBAL['select_realname'][$uid] = $uid;
...
function realname_get ()
{
  ...
  if (empty ($_SGLOBAL['_realname_get']) && $_SCONFIG['realname']
      && $_SGLOBAL['select_realname'])
    {
      ...
      if ($space && isset ($_SGLOBAL['select_realname'][$space['uid']]))
	{
      ...
      if ($_SGLOBAL['member']['uid']
	  && isset ($_SGLOBAL['select_realname'][$_SGLOBAL['member']['uid']]))
	{
	  unset ($_SGLOBAL['select_realname'][$_SGLOBAL['member']['uid']]);
	}
      $uids =
	empty ($_SGLOBAL['select_realname']) ? array () :
	array_keys ($_SGLOBAL['select_realname']);
      if ($uids)
	{
	  $query =
	    $_SGLOBAL['db']->
	    query ("SEL\x45\x43\124 uid, name, namestatus \x46\122\x4fM ".
		   tname ('space')." WHERE uid IN (".simplode ($uids).")");
	  while ($value = $_SGLOBAL['db']->fetch_array ($query))
	    {
...
function getmtag ($id)
{
  global $_SGLOBAL;
  $query =
    $_SGLOBAL['db']->query ("\123\105\114\x45CT * \x46\x52\x4f\x4d ".
			    tname ('mtag')." WHERE tagid='$id'");
  if (!$mtag = $_SGLOBAL['db']->fetch_array ($query))
  ...
  if ($mtag['membernum'] < 1 && ($mtag['joinperm'] || $mtag['viewperm']))
    {
      ...
      updatetable ('mtag', array ('joinperm' = >0, 'viewperm' = >0),
		   array ('tagid' = >$id));
    }
  ...
  $mtag['grade'] = -9;
    ...
    $_SGLOBAL['db']->query ("\123\x45\x4cECT grade F\x52\x4f\115 ".
			    tname ('tagspace').
			    " WHERE tagid='$id' AND uid='$_SGLOBAL[supe_uid]' LIMIT 1");
  if ($value = $_SGLOBAL['db']->fetch_array ($query))
    {
...
function space_key ($space, $appid = 0)
{
  ...
  return
    substr (md5
	    ($_SCONFIG['sitekey'].'|'.
	     $space['uid'].(empty ($appid) ? '' : '|'.$appid)), 8, 16);
}
...
function space_domain ($space)
{
  ...
  if ($space['domain'] && $_SCONFIG['allowdomain'] && $_SCONFIG['domainroot'])
    {
      $space['domainurl'] =
	'http://'.$space['domain'].'.'.$_SCONFIG['domainroot'];
    }
  else
    {
...
function formhash ()
{
  ...
  if (empty ($_SGLOBAL['formhash']))
    {
      $hashadd =
	defined ('IN_ADMINCP') ? 'Only For UCenter Home AdminCP' : '';
      $_SGLOBAL['formhash'] =
	substr (md5
		(substr ($_SGLOBAL['timestamp'], 0, -7).'|'.
		 $_SGLOBAL['supe_uid'].'|'.md5 ($_SCONFIG['sitekey']).'|'.
		 $hashadd), 8, 8);
    }
...
function FNGbgKPZ3 ($mobile)
{
  return strlen (trim ($mobile)) == 11
    && preg_match ("/[13|15|18]{1}[0-9]{9}/", $mobile);
}
...
{
  global $_SGLOBAL, $_SCONFIG;
  if ($_SCONFIG['my_status'] && empty ($_SCONFIG['my_closecheckupdate'])
      && checkperm ('admin'))
    {
      ...
      echo
	'<script type="text/javascript" src="http://notice.uchome.manyou.com/notice?sId='.
...
function g_icon ($gid)
{
  ...
  if (empty ($_SGLOBAL['grouptitle'][$gid]['icon']))
    ...
    {
      echo ' <img src="'.$_SGLOBAL['grouptitle'][$gid]['icon'].
	'" align="absmiddle"> ';
    }
}
...
function ckfounder ($uid)
{
  global $_SC;
  $founders =
    empty ($_SC['founder']) ? array () : explode (',', $_SC['founder']);
  if ($uid && $founders)
    {
...
function getreward ($action, $update = 1, $uid = 0, $needle =
		     '', $setcookie = 1)
{
  ...
  if ($rule['credit'] || $rule['experience'])
      ...
      if ($rule['rewardtype'])
	{
	    ...
	    $_SGLOBAL['db']->query ("\x53\x45\x4c\105CT * FRO\115 ".
				    tname ('creditlog').
				    " WHERE uid='$uid' AND rid='$rule[rid]'");
	  $creditlog = $_SGLOBAL['db']->fetch_array ($query);
	  if (empty ($creditlog))
	    {
	      $reward['credit'] = $rule['credit'];
	      $reward['experience'] = $rule['experience'];
	      $setarr = array ('uid' = >$uid, 'rid' = >$rule['rid'], 'total' =
			       >1, 'cyclenum' = >1, 'credit' =
			       >$rule['credit'], 'experience' =
			       >$rule['experience'], 'dateline' =
			       >$_SGLOBAL['timestamp']);
		...
		case 1:
		  ...
		  if (empty ($rule['rewardnum'])
		      || $creditlog['cyclenum'] < $rule['rewardnum'])
		    {
		      if ($rule['norepeat'])
			{
			  $repeat =
			    checkcheating ($creditlog, $needle,
					   $rule['norepeat']);
			  if ($repeat && !$newcycle)
			    {
			      return $reward;
		      ...
		      $reward['credit'] = $rule['credit'];
		      $reward['experience'] = $rule['experience'];
		      $setarr = array ('cyclenum' =
					>"cyclenum=$sql", 'total' =
					>'total=total+1', 'dateline' =
		...
		case 2:
		  ...
		  if ($creditlog['starttime'])
			...
			{
			  $nextcycle =
			    $creditlog['starttime'] + $rule['cycletime'] * 60;
			}
		    }
		  if ($_SGLOBAL['timestamp'] <= $nextcycle
		      && $creditlog['cyclenum'] < $rule['rewardnum'])
		    {
		  ...
		  elseif ($_SGLOBAL['timestamp'] >= $nextcycle)
		  {
		    ...
		    $reward['experience'] = $rule['experience'];
		    $reward['credit'] = $rule['credit'];
		    $setarr = array ('cyclenum' = >"cyclenum=1", 'total' =
				     >'total=total+1', 'dateline' =
				     >"dateline='$_SGLOBAL[timestamp]'",
				     'credit' =
				     >"credit='$reward[credit]'",
	      ...
	      if ($rule['norepeat'] && $needle)
		{
		  switch ($rule['norepeat'])
		    ...
		    case 0:
		      break;
		    case 1:
		      $info = empty ($creditlog['info'])
			|| $newcycle ? $needle : $creditlog['info'].','.
			$needle;
		      $setarr['info'] = "`info`='$info'";
		      break;
		    case 2:
		      $user = empty ($creditlog['user'])
			|| $newcycle ? $needle : $creditlog['user'].','.
			$needle;
		      $setarr['user'] = "`user`='$user'";
		      break;
		    case 3:
		      $app = empty ($creditlog['app'])
			|| $newcycle ? $needle : $creditlog['app'].','.
			$needle;
		      $setarr['app'] = "`app`='$app'";
		      break;
		    }
	      ...
	      if ($setarr)
		{
		  $_SGLOBAL['db']->query ("UPDATE ".tname ('creditlog').
					  " SET ".implode (',',
							   $setarr).
					  " WHERE clid='$creditlog[clid]'");
		}
	    }
  ...
  return array ('credit' = >abs ($reward['credit']), 'experience' =
		>abs ($reward['experience']));
}
  ...
  if ($topicid)
    {
      ...
      $query =
	$_SGLOBAL['db']->query ("\x53E\x4c\105\x43\x54 * \x46R\x4f\x4d ".
				tname ('topic')." WHERE topicid='$topicid'");
      if ($topic = $_SGLOBAL['db']->fetch_array ($query))
	{
	  $topic['pic'] =
	    $topic['pic'] ? pic_get ($topic['pic'], $topic['thumb'],
				     $topic['remote'], 0) : '';
	  $topic['joingid'] =
	    empty ($topic['joingid']) ? array () : explode (',',
							    $topic
							    ['joingid']);
	  $topic['jointype'] =
	    empty ($topic['jointype']) ? $typearr : explode (',',
							     $topic
							     ['jointype']);
	  $topic['lastpost'] = sgmdate ('Y-m-d H:i', $topic['lastpost']);
	  $topic['dateline'] = sgmdate ('Y-m-d H:i', $topic['dateline']);
	  $topic['allowjoin'] = $topic['endtime']
	    && $_SGLOBAL['timestamp'] > $topic['endtime'] ? 0 : 1;
	  $topic['endtime'] =
	    $topic['endtime'] ? sgmdate ('Y-m-d H:i', $topic['endtime']) : '';
	  include_once (S_ROOT.'./source/function_bbcode.php');
	  ...
	  foreach ($typearr as $value)
	  {
		...
		if ($value == 'pic')
		  $value = 'upload';
		$topic['joinurl'] =
		  "cp.\x70\x68\160?ac=$value&topicid=$topicid";
		break;
	      }
	  }
	}
...
function ckspacelog ()
{
  ...
  if (empty ($_SGLOBAL['supe_uid']))
    return false;
  $query =
    $_SGLOBAL['db']->query ("S\x45\x4c\x45\x43\124 * \106\x52\117\115 ".
			    tname ('spacelog').
			    " WHERE uid='$_SGLOBAL[supe_uid]'");
  if ($value = $_SGLOBAL['db']->fetch_array ($query))
    {
      if ($value['expiration']
	  && $value['expiration'] <= $_SGLOBAL['timestamp'])
	{
	  $_SGLOBAL['db']->query ("DELETE \x46RO\115 ".tname ('spacelog').
				   " WHERE uid='$_SGLOBAL[supe_uid]'");
	}
      $expiration = sgmdate ('Y-m-d H:i', $value['expiration']);
      showmessage ('no_authority_expiration'.
		   ($value['expiration'] ? '_date' : ''), '', 1,
		   array ($expiration));
    }
}

function wap_seccode ()
{
  return '<img src="do.php?ac=seccode&rand='.mt_rand ().'" alt="seccode"/>';
}

function jTtcgMMUE ($content, $out_charset, $in_charset)
{
  global $_SCONFIG, $_SC;
  if ($out_charset == $in_charset)
      ...
      if (!empty ($content) && is_array ($content))
	{
	  ...
	  {
	    $content[$key] = jTtcgMMUE ($value, $out_charset, $in_charset);
	  }
	}
      else
	{
	  if (!empty ($content) && is_string ($content)
	    ...
	    {
	      $content = siconv ($content, $out_charset, $in_charset);
	    }
	}
    }
  return $content;
