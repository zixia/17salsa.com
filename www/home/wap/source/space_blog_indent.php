< ? php if (!defined ('IN_UCHOME') || !defined ('qVh0gqGnK'))
  {
    exit ('Access Denied');
  }
...
$clicks =
empty ($_SGLOBAL['click']['blogid']) ? array () :
$_SGLOBAL['click']['blogid'];
if ($id)
  {
    $query =
      $_SGLOBAL['db']->query ("\123EL\x45\x43T bf.*, b.* \x46RO\x4d ".
			      tname ('blog')." b LEFT JOIN ".
			      tname ('blogfield').
			      " bf \117N bf.blogid=b.blogid WHERE b.blogid='$id' AND b.uid='$space[uid]'");
      ...
      if ($cookievalue != md5 (md5 ($blog['password'])))
	{
      ...
      empty ($blog['tag']) ? array () : unserialize ($blog['tag']);
    include_once (S_ROOT.'./source/function_blog.php');
    $blog['message'] = blog_bbcode ($blog['message']);
    $otherlist = $newlist = array ();
    if ($_SCONFIG['uc_tagrelatedtime']
	 && ($_SGLOBAL['timestamp'] - $blog['relatedtime'] >
	     $_SCONFIG['uc_tagrelatedtime']))
      {
	$blog['related'] = array ();
      }
	...
	if (!empty ($_SCONFIG['uc_tagrelated']) && $_SCONFIG['uc_status'])
	  {
		...
		$blog['related'] =
		  uc_tag_get ($b_tags[$tag_index],
			      $_SGLOBAL['tagtpl']['limit']);
	      }
	  }
	else
	  {
	    ...
	    $query =
	      $_SGLOBAL['db']->
	      query ("\123E\x4c\x45\103\x54 DISTINCT blogid \106\x52O\x4d ".
		     tname ('tagblog')." WHERE tagid IN (".
		     simplode ($b_tagids).
		...
		$query =
		  $_SGLOBAL['db']->
		  query
		  ("\x53\x45\114\x45\103T uid,username,subject,blogid F\x52OM ".
		   tname ('blog')." WHERE blogid IN (".
		   simplode ($tag_blogids).")");
		while ($value = $_SGLOBAL['db']->fetch_array ($query))
		  {
		    realname_set ($value['uid'], $value['username']);
		    $value['url'] =
		      "space.p\x68\x70?uid=$value[uid]&d\x6f=blog&id=$value[blogid]";
	...
	if (!empty ($blog['related']) && is_array ($blog['related']))
	  {
	    foreach ($blog['related'] as $appid = >$values)
	    {
		  ...
		  && $_SGLOBAL['tagtpl']['data'][$appid]['template'])
		{
		  foreach ($values['data'] as $itemkey = >$itemvalue)
		  {
			...
			$blog['related'][$appid]['data'][$itemkey]['html'] =
			  stripslashes (str_replace
					($searchs, $replaces,
					 $_SGLOBAL['tagtpl']['data'][$appid]
					 ['template']));
		      }
		    else
		      {
	...
	updatetable ('blogfield',
		     array ('related' =
			    >addslashes (serialize
					 (sstripslashes ($blog['related']))),
			    'relatedtime' =
			    >$_SGLOBAL['timestamp']), array ('blogid' =
							     >$blog
							     ['blogid']));
      ...
      {
	$blog['related'] =
	  empty ($blog['related']) ? array () :
	  unserialize ($blog['related']);
      }
    $otherlist = array ();
    $query =
      $_SGLOBAL['db']->query ("\x53\105LE\x43T * FR\117\x4d ".tname ('blog').
    ...
    $query =
      $_SGLOBAL['db']->query ("S\105\114\x45C\x54 * F\122OM ".tname ('blog').
	...
	$query =
	  $_SGLOBAL['db']->
	  query ("\123\105\x4c\105\x43\124 * \x46\122\x4f\x4d ".
		 tname ('comment').
		 " WHERE $csql id='$id' AND idtype='blogid' ORDER BY dateline DESC LIMIT $start,$perpage");
	while ($value = $_SGLOBAL['db']->fetch_array ($query))
	  {
      ...
      multi ($count, $perpage, $page,
	     "space.\160\x68\160?uid=$blog[uid]&\144o=$do&id=$id", '',
	     'content');
    ...
    $query =
      $_SGLOBAL['db']->query ("S\x45\114\x45\x43T * F\122OM ".
			      tname ('clickuser')."
...
else
  {
    ...
    if ($_GET['view'] == 'click')
      {
	$theurl = "space.p\150\x70?uid=$space[uid]&d\157=$do&view=click";
	$actives = array ('click' = >' class="active"', 'view' = >'click');
	...
	$count =
	  $_SGLOBAL['db']->result ($_SGLOBAL['db']->
				   query
				   ("S\105\x4c\105\x43T COUNT(*) \x46\122\x4f\115 ".
				    tname ('clickuser').
	...
	if ($count)
	  {
	    $query =
	      $_SGLOBAL['db']->query ("S\105\x4cE\x43T b.*, bf.message, bf.target_ids, bf.magiccolor F\122\x4fM ".tname ('clickuser')." c
				LEFT JOIN ".tname ('blog')." b \x4f\116 b.blogid=c.id
				LEFT JOIN ".tname ('blogfield').
				      " bf \117\116 bf.blogid=c.id
	...
	if ($_GET['view'] == 'all')
	  {
	    ...
	    $actives = array ('all' = >' class="active"', 'view' = >'all');
		...
		$theurl =
		  "space.\160\x68\x70?uid=$space[uid]&d\157=blog&view=all&orderby=$_GET[orderby]";
		$all_actives = array ($_GET['orderby'] = >' class="current"');
		if ($_GET['day'])
		  {
		    $_GET['hotday'] = $_GET['day'];
		    $daytime =
		      $_SGLOBAL['timestamp'] - $_GET['day'] * 3600 * 24;
	      ...
	      {
		$theurl =
		  "space.\x70h\160?uid=$space[uid]&d\x6f=$do&view=all";
	  ...
	  {
	    if (empty ($space['feedfriend']) || $classid)
	      $_GET['view'] = 'me';
	    if ($_GET['view'] == 'me')
	      {
		$wheresql = "b.uid='$space[uid]'";
		$theurl =
		  "space.\x70\150\160?uid=$space[uid]&\x64o=$do&view=me";
		$actives = array ('me' = >' class="active"', 'view' = >'me');
		$query =
		  $_SGLOBAL['db']->
		  query ("S\105L\x45CT classid, classname F\122\117\x4d ".
			 tname ('class')." WHERE uid='$space[uid]'");
		while ($value = $_SGLOBAL['db']->fetch_array ($query))
		  {
		...
		$theurl =
		  "space.\x70\150\160?uid=$space[uid]&d\157=$do&view=we";
		...
		if ($fuid && in_array ($fuid, $space['friends']))
		  {
		    $wheresql = "b.uid = '$fuid'";
		    $theurl =
		      "space.\160\x68\x70?uid=$space[uid]&do=$do&view=we&fuid=$fuid";
		    $f_index = '';
		    $fuid_actives = array ($fuid = >' selected');
		  }
		$actives = array ('we' = >' class="active"', 'view' = >'we');
		$query =
		  $_SGLOBAL['db']->
		  query ("\x53\105LE\103T * \x46\122\x4f\x4d ".
			 tname ('friend').
			 " WHERE uid='$space[uid]' AND \x73\164\141tu\x73='1' ORDER BY num DESC, dateline DESC LIMIT 0,500");
		while ($value = $_SGLOBAL['db']->fetch_array ($query))
		  {
	...
	$count =
	  $_SGLOBAL['db']->result ($_SGLOBAL['db']->
				   query
				   ("S\x45\114E\x43\124 COUNT(*) \x46\x52O\115 ".
				    tname ('blog')." b WHERE $wheresql"), 0);
	if ($wheresql == "b.uid='$space[uid]'"
	     && $space['blognum'] != $count)
	  {
	    updatetable ('space', array ('blognum' = >$count),
			 array ('uid' = >$space['uid']));
	  }
	if ($count)
	  {
	    $query =
	      $_SGLOBAL['db']->query ("\123\x45\x4c\105\x43\x54 bf.message, bf.target_ids, bf.magiccolor, b.* \x46RO\115 ".tname ('blog')." b $f_index
				LEFT JOIN ".tname ('blogfield').
				      " bf \117N bf.blogid=b.blogid
				WHERE $wheresql
	...
	while ($value = $_SGLOBAL['db']->fetch_array ($query))
	  {
	    if (ckfriend
		($value['uid'], $value['friend'], $value['target_ids']))
	      {
		realname_set ($value['uid'], $value['username']);
		if ($value['friend'] == 4)
		  {
		  ...
		  {
		    $value['message'] =
		      getstr ($value['message'], $summarylen, 0, 0, 0, 0, -1);
		  }
		if ($value['pic'])
		  $value['pic'] =
