...
function addFileItem(file) {
  ...
  _del_button.href  = 'javascript:delFile('+file.file_id+',\''+file.file_name+'\');';
...
function delFile(file_id, filePath) {
    ...
    if (r.done) {
      ...
      for(var i=0; i<frames.length; i++) {
        if (frames[i].className.indexOf("nicEdit-frame")>=0) {
          try
          {
            var elem = frames[i].contentWindow.document.getElementsByTagName("img");
            for (i=0; i < elem.length; i++)
            {
                if (elem[i].src.indexOf(filePath) != -1)
                {
...
var nicEditorInstance = bkClass.extend({
  ...
  construct: function(F, D, C) {
    ...
    if (G || this.options.hasPanel) {
      ...
      if (G) {
        if (this.ne.options.needChange){
          if (this.ne.options.bbCode){
            F.value = bbcode.html2bbCode(F.value);
          }else{
            F.value = bbcode.bbcode2Html(F.value);
          }
        }
        if (this.ne.options.bbCode) {
