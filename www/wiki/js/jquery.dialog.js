...
$._dialog = {
	...
	init : function(){
		...
		self.skins['default']={
			...
			'handle':'height:25px;background:url('+self.config.base+'/bg_box_hand.gif);cursor:move;'
			...
			'close':'display:block;float:right;cursor:pointer;width:22px;height:22px;'
		...
		self.skins['lightbox'] = {
			...
			'close':'display:block;float:right;cursor:pointer;width:22px;height:22px;'
				+'background:url('+self.config.base+'/close.jpg)',
			'alphaDiv':'z-index:2001;cursor:default;background:url('+self.config.base+'/overlay.png);'
				+'width:100%;height:100%;top:0px;left:0px;position:absolute;margin:0 auto;',
			'alphaOpacity':0.7
		}
		*/
	...
	build : function(options){
		...
		if (!dialog){
			...
			html +='<tr><td name="content" style="'+ skin['content']+'"></td></tr></table>'
			+ '</div>';
...
$.extend($.dialog, {
	...
	alert:function(content, title, autoClose){
		...
		var options = {id:'jquery_dialog_tip', title:title, position:'c',content:content,height:160,autoClose:autoClose};
		$._dialog.open(options);
	},
