...
$(document).ready(function(){
	...
	$("#doc_verification_code").find("span[name=tip]").html("[点击输入框显示验证码]");
	$("#onloading").hide();
...
var docReference = {
	...
	save: function(){
		...
		if (this.editid == 0){
			...
			$.ajax({
				...
				data:{'data[id]':self.editid, 'data[name]':name, 'data[url]':url},
				type:'POST',
	...
	add: function(value){
		...
		$.ajax({
			url:'index.php?reference-add',
			data:{'data[name]':name, 'data[url]':url, 'data[did]':g_docid},
			type:'POST',
