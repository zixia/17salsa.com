...
class picmodel {
	...
	function get_pic($type=1,$start=0,$limit=20){
		...
		$sql=" SELECT p.*,d.title  FROM ".DB_TABLEPRE."attachment p left join ".DB_TABLEPRE."doc d ON p.did=d.did  WHERE state=0  AND isimage=1 ".$pictype[$type]." LIMIT $start,$limit";
	...
	function search_pic_num($keyword){
		$sql="SELECT count(*) num FROM ".DB_TABLEPRE."attachment p LEFT JOIN ".DB_TABLEPRE."doc d ON p.did=d.did WHERE d.title LIKE '%$keyword%' OR p.description LIKE '%$keyword%' ORDER BY p.time DESC";
		return $this->db->result_first($sql);
	}
	
	function search_pic($keyword,$start=0,$limit=15){
		$piclist=array();
		$sql=" SELECT p.*,d.title,d.author,d.summary FROM ".DB_TABLEPRE."attachment p LEFT JOIN ".DB_TABLEPRE."doc d ON p.did=d.did WHERE d.title LIKE '%$keyword%' OR p.description LIKE '%$keyword%' ";
	...
	function get_pic_by_did($did){
		$piclist=array();
		$sql=" SELECT id,did,attachment  FROM ".DB_TABLEPRE."attachment WHERE did='$did' ";
		$query = $this->db->query($sql);
		while($pic=$this->db->fetch_array($query)){
			$pic_140=$this->get_140($pic['attachment']);
			if(is_file($pic_140)){
