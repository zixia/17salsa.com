...
class categorymodel {
	...
	function get_cat($catid){
		...
		if(is_array($cats))	{
			if($catid == 0){
				foreach($cats as $cat){
					$content .= '<dl class="col-dl"><dt class="bold"><label><input type="checkbox" id='.$cat['cid'].' name='.$cat['name'].' onclick="javascript:catevalue.cateOk('.$cat['cid'].',\''.$cat['name'].'\',this.checked)" />'.$cat['name'].'</label></dt>';
					$subcats = $this->get_subcate($cat['cid']);
					if($subcats){
						$content .= $this->get_catitem($subcats);
					}
					$content .= '</dl>';
				}
			}else{
				...
				foreach (unserialize($cat['navigation']) as $nav){
					$navlink .= '<label><input type="checkbox" id='.$nav['cid'].' name='.$nav['name'].' onclick="javascript:catevalue.cateOk('.$nav['cid'].',\''.$nav['name'].'\',this.checked)" />'.$nav['name'].'</label>>';
				}
				$content .= '<dl class="col-dl"><dt class="bold">'.substr($navlink, 0, -1).'</dt>';
				$content .= $this->get_catitem($cats);
	...
	function get_catitem($cats){
		$content = '';
		foreach ($cats as $cat){
			$img = $this->get_subcate($cat['cid']) ? '<input onclick="javascript:catevalue.ajax('.$cat['cid'].')" type="image" src="style/default/sign_next.gif"/>' : '';
			$content .= '<dd><label><input type="checkbox" id='.$cat['cid'].' name='.$cat['name'].' onclick="javascript:catevalue.cateOk('.$cat['cid'].',\''.$cat['name'].'\',this.checked)"/>'.$cat['name'].'</label>'.$img.'</dd>';
		}
		return $content;
	}
}
