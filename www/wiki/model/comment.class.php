...
class commentmodel {
	...
	function remove_comment_by_id($id){
		...
		while($comment=$this->db->fetch_array($query)){
			$sqladd.=" ('comment','".$comment['comment']."','".addslashes(serialize($comment))."','N;','".$this->base->user['uid']."','".$this->base->user['username']."','".$this->base->time."'),";
		}
