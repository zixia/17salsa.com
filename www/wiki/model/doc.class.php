...
class docmodel {
	...
	function edit_doc($doc,$edittype='1') {
		...
		VALUES ('".$edition['did']."','".$this->base->user['username']."','".$this->base->user['uid']."',
		'".$edition['lastedit']."','".$this->base->ip."','".$edition['title']."','".$edition['tags']."','".$edition['summary']."','".$edition['content']."','".string::hstrlen($edition['content'])."','".util::getimagesnum($edition['content'])."','".$doc['reason']."','$edittype')");
	}
	...
	function get_edition_list($did, $field='*', $max_eid=0){
		$editionlist=array();
		$sql = " SELECT $field FROM ".DB_TABLEPRE."edition WHERE did=$did ";
		if ($max_eid){
	...
	function audit_doc($dids){
		foreach($dids as $did){
			...
			if($doc['authorid']&&$doc['editions']>0){
				$_ENV['user']->add_credit($doc['lasteditorid'],'doc-edit',$this->base->setting['credit_edit']);
			}elseif($doc['authorid']&&$doc['editions']==0){
				$_ENV['user']->add_credit($doc['authorid'],'doc-create',$this->base->setting['credit_create']);
			}
		}
		$dids = implode(',',$dids);
	...
	function remove_doc ($did){
		...
		while($row=$this->db->fetch_array($query)){
			if($row['image']){
				...
				$file[$row['did']][]=str_replace('f_', 's_f_', $row['image']);
			}
		}
		foreach($did as $id){
			...
			$this->db->query("INSERT INTO ".DB_TABLEPRE."recycle (type,keyword,content,file,adminid,admin,dateline) values ('doc','$keyword','".addslashes(serialize($content))."','".serialize($file[$id])."','".$this->base->user['uid']."','".$this->base->user['username']."','".$this->base->time."'); ");
			$keyword="";
			$content="";
		}
	...
	function remove_edition($eid, $did=0){
		...
		$query=$this->db->query("SELECT * FROM ".DB_TABLEPRE."edition WHERE eid IN ($eid)");
		while($edition=$this->db->fetch_array($query)){
			$sql.="('edition','".$edition['title']."','".addslashes(serialize($edition))."','N;','".$this->base->user['uid']."','".$this->base->user['username']."','".$this->base->time."'),";
		}
		$this->db->query(substr($sql,0,-1));

		$this->db->query("DELETE FROM ".DB_TABLEPRE."edition WHERE eid IN ($eid)");
		if($did){$this->db->query("UPDATE ".DB_TABLEPRE."doc SET editions=editions-1 WHERE editions>0 AND did=$did");}
		return true;
	}
	...
	function getnews(){
		$newslist = array();
		$query=$this->db->query("SELECT * from ".DB_TABLEPRE."user order by regtime desc limit 10");
		while($userlist=$this->db->fetch_array($query)){
			$newslist[$userlist['regtime']]="<a href='index.php?user-space-".$userlist['uid']."' class='red'>".$userlist['username']."</a>".$this->base->view->lang['news_join'].$this->base->setting['site_name'];
		}
		$query=$this->db->query("SELECT * from ".DB_TABLEPRE."edition order by time desc limit 10");
		while($editionlist=$this->db->fetch_array($query)){
			$newslist[$editionlist['time']]="<a href='index.php?user-space-".$editionlist['authorid']."' class='red'>".$editionlist['author'].'</a>'.$this->base->view->lang['news_edition_tip1'].'<a href="index.php?doc-view-'.$editionlist['did'].'" target="_blank">'.$editionlist['title'].'</a>'.$this->base->view->lang['news_edition_tip2'];
		}
		...
		while($giftlog=$this->db->fetch_array($query)){
			$newslist[$giftlog['time']]='<a href="index.php?user-space-'.$giftlog['uid'].'" class="red">'.$giftlog['username'].'</a>的<font color="blue">'.$giftlog['title'].'</font>兑换申请已经通过，请等候发放。';
		}
		krsort($newslist);
		return $newslist;
	}
