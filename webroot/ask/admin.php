...
$admin_check = $_DCOOKIE['adminhash']==ADMINHASH ? 1:0;
$admin_login = ($cyask_uid && $admin_check) ? 1:0;
...
if(isset($_POST['login_submit']))
{
	if(check_submit($_POST['login_submit'], $_POST['formhash']))
	{
		...
		list($cyask_uid,$cyask_uname,$cyask_pw,$cyask_email)=uc_user_login($username,$password);
		if($cyask_uid > 0)
		{
			$url=empty($url) ? './' : $url;
			
			set_cookie('compound', authcode("$cyask_uid\t$cyask_uname\t$cyask_pw", 'ENCODE', $cyask_key));
			set_cookie('styleid', $styleid);
			set_cookie("adminhash",ADMINHASH);
...
else
{
	if($cyask_adminid == 1)
	{
		...
		$admin_action == 'ques_top' || $admin_action == 'ques_close' || $admin_action == 'knowledge_share')
		{
