...
elseif($command=='answer')
{
	...
	if($page>$pagecount) $page=1;
	$pagestart=($page-1)*$pagerow;
	$query=$dblink->query("select a.aid,a.qid,a.answertime,a.response,q.title,q.status,q.score,q.answercount from {$tablepre}answer a,{$tablepre}question q WHERE a.uid=$uid AND a.qid=q.qid limit $pagestart,$pagerow");
...
elseif($command=='share')
{
	$members=$dblink->fetch_array($query);
	$page=intval($_GET['page']);
	if($page<1) $page=1;
	$pagerow=10;
	$query=$dblink->query("select count(*) from {$tablepre}question where uid=$uid and status=7");
	$quescount=$dblink->result($query,0);     
	$pagecount=ceil($quescount/$pagerow);
	if($page>$pagecount) $page=1;
	$pagestart=($page-1)*$pagerow;
	$query=$dblink->query("select * from {$tablepre}question where uid=$uid and status=7 limit $pagestart,$pagerow");
