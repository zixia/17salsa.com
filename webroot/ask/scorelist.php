...
switch($stype)
{
	...
	$query=$dblink->query("select count(uid) from {$tablepre}score where week=$lastweek group by uid");
	...
	{
		$temp['orderid'] = $i+1;
		$temp['avatar'] = UC_API.'/avatar.php?uid='.$temp[uid].'&size=small';
		$query1 = $dblink->query("select username,lastlogin from {$tablepre}member where uid=$temp[uid]");
	...
	if($page>$pagecount) $page=1;
	$pagestart=($page-1)*$pagerow;
	$query=$dblink->query("select uid,username,allscore as newscore,lastlogin from {$tablepre}member where adminid=5 order by allscore desc limit $pagestart,$pagerow");
	$i=0;
	while($temp=$dblink->fetch_array($query))
	{
		$temp['orderid']=$i+1;
