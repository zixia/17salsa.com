...
class statisticsmodel {
	...
	function star_today(){
		...
		return $this->db->fetch_first("SELECT authorid,author FROM ".DB_TABLEPRE."edition WHERE time >= $time GROUP BY authorid ORDER BY COUNT(authorid) DESC");
	}
	...
	function category_toplist($type,$limit=20){
		...
		case 1:
			...
			break;
		case 2:
			$sql.="FROM ".DB_TABLEPRE."edition d , ".DB_TABLEPRE."categorylink l , ".DB_TABLEPRE."category c WHERE l.did=d.did AND c.cid=l.cid GROUP BY l.cid ORDER BY COUNT(1) DESC ";
			break;
		case 3:
			$time=$this->base->time-3600*24*30;
			$sql.="FROM ".DB_TABLEPRE."doc d , ".DB_TABLEPRE."categorylink l , ".DB_TABLEPRE."category c WHERE l.did=d.did AND c.cid=l.cid AND d.time >= $time GROUP BY l.cid ORDER BY COUNT(1) DESC ";
			break;
		case 4:
			$time=$this->base->time-3600*24;
			$sql.="FROM ".DB_TABLEPRE."doc d , ".DB_TABLEPRE."categorylink l , ".DB_TABLEPRE."category c WHERE l.did=d.did AND c.cid=l.cid AND d.time >= $time GROUP BY l.cid ORDER BY COUNT(1) DESC ";
	...
	function user_toplist($parameter,$type=1,$limit=20){
		...
		if($type===1){
			...
			$sql="SELECT authorid uid,author username,COUNT(authorid) num FROM ".DB_TABLEPRE."doc WHERE time >= $parameter GROUP BY authorid ORDER BY COUNT(authorid) DESC ";
		}
		if((bool)$limit){
			$sql.="limit ".$limit;
		}
