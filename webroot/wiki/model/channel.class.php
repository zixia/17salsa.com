...
class channelmodel {
	...
	function add_channel($channel){
		return $this->db->query("INSERT INTO  ".DB_TABLEPRE."channel (name,url,available) VALUES ('$channel[name]','$channel[url]',$channel[available]) ");
	}
	
	function edit_channel($channel){
		return $this->db->query("UPDATE ".DB_TABLEPRE."channel SET name='$channel[name]',url='$channel[url]',available='$channel[available]' WHERE id=$channel[id] ");
	}
